...
class MobileFaceNet(Module):
    ...
    def forward(self, x, depth=None):
        ...
        assert depth is not None
        out1 = self.conv1(depth)
        out1 = self.conv2_dw(out1)
        out1 = self.conv_23(out1)
        out1 = self.conv_3(out1)
        out1 = self.conv_34(out1)
        out1 = self.conv_4(out1)
        out1 = self.conv_45(out1)
        out1 = self.conv_5(out1)
        out1 = self.conv_6_sep(out1)
        out1 = self.conv_6_dw(out1)
        out1 = self.conv_6_flatten(out1)
        out1 = self.linear(out1)
        out1 = self.bn(out1)

        x = torch.cat([out, out1], dim=1)
        x = self.mlp(x)
...
class Am_softmax(Module):
    ...
    def __init__(self, embedding_size=512, classnum=51332):
        ...
        print('\n\CosFace with the following property')
        print('self.m', self.m)
        print('self.s', self.s)
